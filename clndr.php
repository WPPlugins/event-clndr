...
function CLDNR_init() {
    ...
    load_plugin_textdomain('event-clndr',false, $plugin_dir.'/languages');
    ...
    if ( $installed_CLNDR_VERSION != CLNDR_VERSION ) {
        ...
        $sql = "CREATE TABLE " . CLNDR_EVENTS_TABLE . " (
            ...
            event_link_target VARCHAR(20) DEFAULT '' NOT NULL,
...
function CLNDR_shortcode($atts) {
    ...
    if ($instance) {
        ...
        return __("[ CLNDR instance not found ]", "event-clndr");
    }
                ...
                if ($showThisEvent) {
                    if ($limit == 0 || $limit > $showCounter) {
                        $foot_func["events"] .="{
                            ...
                            url_target: '".$event["event_link_target"]."',
...
function admin_CLNDR_INSTANCES() {
    ...
    if (!empty($action)) {
        ...
        if ($action == "add" || $action == "edit_save" || $action == "delete") {
            ...
            if (($action == "edit_save" || $action == "delete") && !$instance_id) { ?>
                <div class="error"><p><strong><?php _e('Error','event-clndr'); ?>:</strong> <?php _e("Invalid instance ID",'event-clndr'); ?></p></div>
                <?php
            }
            elseif (wp_verify_nonce($_REQUEST['_wpnonce'],'clndr-instances-'.$action.$nonce_end) == false) {	?>
               <div class="error"><p><strong><?php _e('Error','event-clndr'); ?>:</strong> <?php _e("Security check failure, please try again",'event-clndr'); ?></p></div>
                 <?php
            } else {
                if ($action == "add" || $action == "edit_save") {
                    ...
                    $validLocale = (in_array($locale,admin_CLNDR_INSTANCES_get_locales()) ? true : __("The locale is invalid", "event-clndr"));
                    $allPeriodsValid = true;
                    $validLimit = ($limit == '' || ctype_digit($limit)) ? true : __("Max events has to be empty or numeric", "event-clndr");

                    if (is_array($periods)) {
                        foreach ($periods as $tp) {
                            if (!in_array($tp,array(1,2,3))) {
                                $allPeriodsValid = __("One or more time periods are specified incorrectly", "event-clndr");
                            }
                        }
                    ...
                    if ($slug["status"] == "success" && $validLocale === true && $allPeriodsValid === true && $validLimit === true) {
                        ...
                        if ($action == "add") {
                            ...
                            <div class="updated"><p><?php _e('Instance successfully added','event-clndr'); ?></p></div>
                            ...
                            <div class="updated"><p><?php _e('Instance successfully edited','event-clndr'); ?></p></div>
                ...
                // Deal with deleting an event from the database
                    ...
                    ?>
                    <div class="updated"><p><?php _e('Instance successfully deleted','event-clndr'); ?></p></div>
                    <?php
                }
            }
        }
    ...
    <div class="wrap">
        ...
        if ( $action == 'edit' || ($action == 'edit_save' && $CLNDR_input_errors)) 	{
            ?>
            <h2><?php _e('Edit Instance','event-clndr'); ?></h2>
            <?php
            if (!$instance_id) { ?>
                <div class="error"><p><strong><?php _e('Error','event-clndr'); ?>:</strong> <?php _e("Invalid instance ID",'event-clndr'); ?></p></div>
                <?php
            ...
            ?>
            <h2><?php _e('Manage Instances','event-clndr'); ?></h2>
            <?php	admin_CLNDR_INSTANCES_display_instances(); ?>

            <h2><?php _e('Add Instance','event-clndr'); ?></h2>
...
function admin_CLNDR_INSTANCES_verify_instance_name($name,$instance_id=false) {
    ...
    if ($name == "") {
        return array("status"=>"error","error"=>__("Instance name cannot be empty","event-clndr"));
    }
    ...
    if (!empty($instances)) {
        ...
        if ($keyInArray !== false && $keyInArray !== $instance_id) {
            return array("status"=>"error","error"=>__("Instance name already taken","event-clndr"));
        }
...
weekOffset: 1';
    ...
    $wp_CLNDR_time_periods = array(1 => __("Past events", "event-clndr"), 2 => __("Current events", "event-clndr"), 3 => __("Future events", "event-clndr"));
        ...
        <div class="postbox">
            <div style="float: left; width: 98%; clear: both;" class="inside">
                <table cellpadding="5" cellspacing="5" width="100%">
                    <tr>
                        <td width="150">
                            <legend for="instance_name" style="display:inline;"><?php __('Name','event-clndr'); ?></legend> <span style="color:red;">*</span>
                        </td>
                    ...
                    <tr>
                        <td style="vertical-align:top;">
                            <legend for="instance_template" style="display:inline;"><?php __('HTML Template','event-clndr'); ?></legend><br />
                            <br />
                            <strong><?php _e("Reference","event-clndr"); ?></strong><br />
                            <a href="https://github.com/kylestetz/CLNDR#introduction-you-write-the-markup" target="_blank"><?php _("CLNDR.js Markup","event-clndr"); ?></a><br />
                            <a href="http://momentjs.com/docs/#/parsing/string-format/" target="_blank"><?php __("Moment.js Formatting","event-clndr"); ?></a>
                        </td>
                    ...
                    <tr>
                        <td style="vertical-align:top;">
                            <legend for="instance_options" style="display:inline;"><?php __('JS Object Options','event-clndr'); ?></legend><br />
                            <span style="display:block;padding-top:10px;font-size:0.9em;line-height:1.2;d">
                                (<?php __('No need to include event and template data, this is automatically populated','event-clndr'); ?>)<br />
                            </span>
                            <br />
                            <strong><?php __("Reference","event-clndr"); ?></strong><br />
                            <a href="https://github.com/kylestetz/CLNDR#usage" target="_blank"><?php __("CLNDR.js Options","event-clndr"); ?></a><br />
                            <br />
                            <strong><?php __("Disclaimer","event-clndr"); ?></strong><br />
                            <span style="font-size:0.9em;line-height:1.2;">
                                <?php __("It is recommended to only add Javascipt options here if you are a web developer, or generally know what you are doing. Incorrectly formatted input could hinder other Javascript functionality on your Wordpress website.","event-clndr"); ?>
                            </span>
                        </td>
                    ...
                    <tr>
                        <td>
                            <legend for="instance_locale" style="display:inline;"><?php _e('Locale','event-clndr'); ?></legend>
                        </td>
                        <td>
                    ...
                    <tr>
                        <td style="vertical-align:top;">
                            <legend for="instance_periods[]" style="display:inline;"><?php _e('Show...','event-clndr'); ?></legend>
                        </td>
                        <td>
                            <?php
                    ...
                    <tr>
                        <td>
                            <legend for="instance_limit" style="display:inline;"><?php _e('Max no. of events','event-clndr'); ?></legend>
                        </td>
                        <td><input type="text" style="width:50px;" name="instance_limit" value="<?php echo $wp_CLNDR_limit; ?>" /> &nbsp;&nbsp;(<?php _e('Leave blank for unlimited','event-clndr'); ?>) </td>
                    </tr>
                </table>
            </div>
            <div style="clear:both;"></div>
        </div>
        <input type="submit" name="save" class="button button-primary button-large" value="<?php _e('Save','event-clndr'); ?>" />
    </form>
    <?php
}
...
function admin_CLNDR_INSTANCES_display_instances(){
    ...
    if ( !empty($instances) ) {
    ?>
        <p><?php __("CLNDR shortcodes can be added into any page or post, as well as into the content of a 'Text' widget for sidebar or footer use.","event-clndr"); ?><p>

        <?php // if admin etc {

            $styles_url = get_bloginfo('wpurl') . "/wp-admin/admin.php?page=clndr-styles";
            $styles_text = sprintf( wp_kses( __(
                'Individual CLNDR instances can be accessed directly from the <a href="%s">master stylesheet</a>. Their shortcode ID is applied <b>as a class</b> to the parent container, prepended by "clndr_".</p>', 'event-clndr'
        ...
        <table class="widefat page fixed" width="100%" style="width:100%;max-width:900px;" cellpadding="3" cellspacing="3">
            <tr>
                <th class="manage-column" scope="col"><?php __('Name','event-clndr') ?></th>
                <th class="manage-column" scope="col"><?php __('Shortcode','event-clndr') ?></th>
            ...
            <?php
                ...
                foreach ( $instances as $instance )	{
                    ...
                    <tr class="<?php echo $class; ?>">
                        ...
                        <td><a href="<?php echo $wp_CLNDR_edit_url; ?>" class='edit'><?php echo __('Edit','event-clndr'); ?></a></td>
                        <td><a href="<?php echo $wp_CLNDR_delete_url; ?>" class="delete" onclick="return confirm('<?php __('Are you sure you want to delete this instance? Any active shortcodes will invalidate','event-clndr'); ?>')"><?php echo __('Delete','event-clndr'); ?></a></td>
                    </tr>
                <?php
                }
            ?>
        ...
        ?><p><?php __("There are no instances in the database",'event-clndr');?></p><?php
    }
}
...
function admin_CLNDR_EVENTS() {
    ...
    $link_target = !empty($_REQUEST['event_link_target']) ? $_REQUEST['event_link_target'] : '';
    ...
    if (!empty($action)) {
        ...
        if ($action == "add" || $action == "edit_save" || $action == "delete") {
            ...
            if (($action == "edit_save" || $action == "delete") && !$event_id) { ?>
                <div class="error"><p><strong><?php _e('Error','event-clndr'); ?>:</strong> <?php _e("Invalid event ID",'event-clndr'); ?></p></div>
                <?php
            }
            elseif (wp_verify_nonce($_REQUEST['_wpnonce'],'clndr-events-'.$action.$nonce_end) == false) {	?>
               <div class="error"><p><strong><?php _e('Error','event-clndr'); ?>:</strong> <?php _e("Security check failure, please try again",'event-clndr'); ?></p></div>
                ...
                if ($action == "add" || $action == "edit_save") {
                    ...
                    $verify = admin_CLNDR_EVENTS_validate_event_fields($title,$desc,$begin,$end,$time,$link,$link_target);

                    if ($verify["status"] == "success") {

                        $sql_main = CLNDR_EVENTS_TABLE . " SET event_title='%s', event_desc='%s', event_begin='%s', event_end='%s', event_time='%s', event_link='%s', event_link_target='%s'";
                        ...
                        if ($action == "add") {
                            ...
                            $sql = $wpdb->prepare(
                                ...
                                $db_vars["title"],$db_vars["desc"],$db_vars["begin"],$db_vars["end"],$db_vars["time"],$db_vars["link"],$db_vars["link_target"]
                            );
                            $wpdb->get_results($sql);
                            ?>

                            <div class="updated"><p><?php _e('Event successfully added. It will now show in your calendar','event-clndr'); ?></p></div>
                            ...
                            <div class="updated"><p><?php _e('Event successfully edited','event-clndr'); ?></p></div>
                        ...
                        $users_entries->event_link_target = admin_CLNDR_sanitize_input_field($link_target);
                        $CLNDR_input_errors = true;
                        ?>

                        <div class="error">
                            <p><strong><?php _e("Please address the below error(s) ",'event-clndr'); ?></strong></p>
                            <ul>
                ...
                // Deal with deleting an event from the database
                    ...
                    ?>
                    <div class="updated"><p><?php _e('Event successfully deleted','event-clndr'); ?></p></div>
                    <?php
                }
            }
        }
    ...
    <div class="wrap">
        ...
        if ( $action == 'edit' || ($action == 'edit_save' && $CLNDR_input_errors)) 	{
            ?>
            <h2><?php _e('Edit Event','event-clndr'); ?></h2>
            <?php
            if (!$event_id) { ?>
                <div class="error"><p><strong><?php _e('Error','event-clndr'); ?>:</strong> <?php _e("Invalid event ID",'event-clndr'); ?></p></div>
                <?php
            ...
            ?>
            <h2><?php _e('Add Event','event-clndr'); ?></h2>
            <?php admin_CLNDR_EVENTS_edit_form("add"); ?>

            <h2><?php _e('Manage Events','event-clndr'); ?></h2>
...
// Function to add additional JS functionality on event form
function admin_CLNDR_EVENTS_load_js() {
    add_action("admin_print_footer_scripts", function() {
        ?>
        <script>
            jQuery("input[name='event_link']").on('input',function(e){
                var v = jQuery(this).val();
                if (v == '') {
                    console.log("dis");
                    jQuery("select[name='event_link_target']").prop('disabled', true);
                } else {
                    console.log("not dis");
                    jQuery("select[name='event_link_target']").prop('disabled', false);
                }
            });
        </script>
        <?php
...
function admin_CLNDR_EVENTS_display_events(){
    ...
    ?>
        <table class="widefat page fixed" width="100%" style="width:100%;max-width:1300px;" cellpadding="3" cellspacing="3">
            <tr>
                <th class="manage-column" scope="col"><?php _e('Title','event-clndr') ?></th>
                <th class="manage-column" scope="col"><?php _e('Start Date','event-clndr') ?></th>
                <th class="manage-column" scope="col"><?php _e('End Date','event-clndr') ?></th>
                <th class="manage-column" scope="col"><?php _e('Time','event-clndr') ?></th>
                <th class="manage-column" scope="col" width="50">&nbsp;</th>
                <th class="manage-column" scope="col" width="50">&nbsp;</th>
            </tr>
            <?php
                ...
                foreach ( $events as $event )	{
                    ...
                    <tr class="<?php echo $class; ?>">
                        ...
                        <td><a href="<?php echo $wp_CLNDR_edit_url; ?>" class='edit'><?php echo __('Edit','event-clndr'); ?></a></td>
                        <td><a href="<?php echo $wp_CLNDR_delete_url; ?>" class="delete" onclick="return confirm('<?php __('Are you sure you want to delete this event?','event-clndr'); ?>')"><?php echo __('Delete','event-clndr'); ?></a></td>
                    </tr>
                <?php
                }
            ?>
        ...
        __("There are no events in the database",'event-clndr');
    }
}
...
function admin_CLNDR_EVENTS_edit_form($wp_CLNDR_mode='add', $wp_CLNDR_event_id=false) {
    ...
    $wp_CLNDR_link_target = (isset($data->event_link_target) ? $data->event_link_target : "");
        ...
        <div class="postbox">
            <div style="float: left; width: 98%; clear: both;" class="inside">
                <table cellpadding="5" cellspacing="5" style="width:100%;">
                    <tr>
                        <td style="width:130px;"><legend for="event_title" style="display:inline;"><?php _e('Event Title','event-clndr'); ?></legend> <span style="color:red;">*</span></td>
                    ...
                    <tr>
                        <td style="vertical-align:top;">
                            <legend for="event_desc" style="display:inline;"><?php _e('Event Description','event-clndr'); ?></legend>
                            <div style="font-size:0.9em;line-height:1.2;padding-top:5px;">
                                <?php _e("Allowable HTML tags: ","event-clndr"); ?>&lt;b&gt;&lt;i&gt;&lt;u&gt;&lt;img&gt;
                            </div>
                        </td>
                    ...
                    <tr>
                        <td><legend for="event_begin" style="display:inline;"><?php _e('Start Date','event-clndr'); ?></legend> <span style="color:red;">*</span></td>
                    ...
                    <tr>
                        <td><legend for="event_end" style="display:inline;"><?php _e('End Date','event-clndr'); ?></legend> <span style="color:red;">*</span></td>
                    ...
                    <tr>
                        <td><legend for="event_time" style="display:inline;"><?php _e('Time','event-clndr'); ?></legend></td>
                    ...
                    <tr>
                        <td><legend for="event_link" style="display:inline;"><?php _e('Event Link','event-clndr'); ?></legend></td>
                    ...
                    <tr>
                        <td><legend for="event_link_target" style="display:inline;"><?php _e('Event Link Target','event-clndr'); ?></legend></td>
                        <td>
                            <select id="event_link_target" name="event_link_target" style="width:100%;max-width:320px;" <?php if ($wp_CLNDR_link == '') { echo "disabled"; } ?>>
                                <option value="_self" <?php if ($wp_CLNDR_link_target == "_self") { echo 'selected'; } ?>><?php __("Same window/tab","event-clndr"); ?></option>
                                <option value="_blank" <?php if ($wp_CLNDR_link_target == "_blank") { echo 'selected'; } ?>><?php __("New window/tab","event-clndr"); ?></option>
                                <option value="clndr" <?php if ($wp_CLNDR_link_target == "clndr") { echo 'selected'; } ?>><?php __("Unique window/tab (open all links in this plugin in the same external window/tab)","event-clndr"); ?></option>
                            </select>
                        </td>
                    </tr>
                </table>
...
*/
function admin_CLNDR_EVENTS_validate_event_fields($title,$desc,$begin,$end,$time,$link,$link_target) {

    $errors = array();

    if ($title == "") {
        $errors[] = __("The title field is empty",'event-clndr');
    } elseif (strlen($title) > 50) {
        $errors[] = __("The title field can only contain 50 characters or less",'event-clndr');
    } else {
    ...
    if (count($beginParts) != 3) {
        $errors[] = __("The start date is empty, or not in the correct format",'event-clndr');
    } else {
        $beginDateIsValid = checkdate($beginParts[1], $beginParts[2], $beginParts[0]);
        if (!$beginDateIsValid) {
            $errors[] = __("The start date is not valid",'event-clndr');
        }
    }
    ...
    if (count($endParts) != 3) {
        $errors[] = __("The end date is empty, or not in the correct format",'event-clndr');
    } else {
        $endDateIsValid = checkdate($endParts[1], $endParts[2], $endParts[0]);
        if (!$endDateIsValid) {
            $errors[] = __("The end date is not valid",'event-clndr');
            ...
            if (mktime(0,0,0,$endParts[1], $endParts[2], $endParts[0]) < mktime(0,0,0,$beginParts[1], $beginParts[2], $beginParts[0])) {
                $errors[] = __("The end date cannot be before the start date",'event-clndr');
            }
        }
    }

    if (strlen($time) > 100) {
        $errors[] = __("The time field can only contain 100 characters or less",'event-clndr');
    } else {
        $time = admin_CLNDR_sanitize_input_field($time);
    }

    if (strlen($link) > 2048) {
        $errors[] = __("The link field can only contain 2,048 characters or less",'event-clndr');
    } else {
        $link = admin_CLNDR_sanitize_input_field($link);
    }

    if ($link_target != "" && $link_target != "_self" && $link_target != "_blank" && $link_target != "clndr") {
        $errors[] = __("You did not select a valid link target",'event-clndr');
    }

    if (empty($errors)) {
        $desc = admin_CLNDR_sanitize_input_field($desc,true);
        return array("status"=>"success","db_vars"=>array("title"=>$title,"desc"=>$desc,"begin"=>$begin,"end"=>$end,"time"=>$time,"link"=>$link,"link_target"=>$link_target));
    } else {
...
function admin_CLNDR_STYLES() {
    ...
    <div class="wrap">
        ...
        <h2><?php _e('Edit Master Stylesheet','event-clndr'); ?></h2>
        <h4><?php echo "(" . $file . ")" ?></h4>
            ...
            if ($_GET["page"] == "clndr-styles" && $_POST["action"] == "update") {
                if ( is_writeable( $file ) ) {
                    ...
                    if ( $f !== false ) {
                        ...
                        fclose( $f );
                        echo '<div class="updated" style="margin-bottom:20px;"><p>' . __("Successfully updated the stylesheet","event-clndr") . '</p></div>';
                    }
                } else {
                    echo '<div class="error" style="margin-bottom:20px;"><p>' . __("Error. The stylesheet does not have the correct permissions to be writeable","event-clndr") . '</p></div>';
                }
            }
        }
        ...
        <form name="template" action="admin.php?page=clndr-styles" method="post">
            ...
            <div>
                <?php
                if (is_writeable( $file )) { ?>
                    <input type="submit" name="submit" class="button button-primary button-large" value="<?php _e('Update File','event-clndr'); ?>" />
                <?php
                } else {
                    $codex_url = "https://codex.wordpress.org/Changing_File_Permissions";
                    $error_text = sprintf( wp_kses( __(
                        'You need to make this file writeable before you can save your changes. See <a href="%s">the Codex</a> for more information.', 'event-clndr'
